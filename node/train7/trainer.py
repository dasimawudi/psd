...
from train7.models import PointMLP
from train7.runtime import ensure_dir, make_logger, write_json, write_yaml
from train7.scalers import RunningTensorStats, StandardScaler


HISTORY_BASE_FIELDS = ["epoch", "train_loss", "val_loss"]

ModelOutput = torch.Tensor | dict[str, torch.Tensor]
...
EVALUATION_DIAGNOSTIC_FIELDS = [
    ...
    "under_pred_ratio",
    "over_pred_ratio",
    "false_peak_top5_rate",
    "false_peak_top10_rate",
    "hotspot_false_peak_margin_log",
    "near_zero_points",
    "near_zero_over_cap_rate",
    "near_zero_over_log_mae",
]
...
def _rank_fraction_from_name(name: str, default: float) -> float:
    normalized = str(name).strip().lower()
    if normalized in {"target_top1", "top1"}:
        return 0.01
    if normalized in {"target_top5", "top5"}:
        return 0.05
    if normalized in {"target_top10", "top10"}:
        return 0.10
    if normalized.startswith("target_top") or normalized.startswith("top"):
        suffix = normalized.rsplit("top", 1)[-1].replace("%", "")
        try:
            return max(float(suffix) / 100.0, 1.0e-6)
        except ValueError:
...
def _model_output_tensor(output: ModelOutput) -> torch.Tensor:
    if isinstance(output, dict):
        return output["stress_scaled"]
    return output


def _effective_prediction_scaled(
    output: ModelOutput,
    y_scaler: StandardScaler,
    sample_index: torch.Tensor | None = None,
    peak_relative_cfg: dict[str, Any] | None = None,
    zero_gate_cfg: dict[str, Any] | None = None,
) -> torch.Tensor:
    base_scaled = _model_output_tensor(output)
    if not isinstance(output, dict):
        return base_scaled

    y_scaler = y_scaler.to(base_scaled.device)
    pred_log = y_scaler.inverse_transform(base_scaled)
    peak_cfg = dict(peak_relative_cfg or {})
    if bool(peak_cfg.get("enabled", False)) and bool(peak_cfg.get("combine_prediction", False)):
        if sample_index is None:
            raise ValueError("sample_index is required when stress_peak_relative.combine_prediction is enabled.")
        peak_scaled = output.get("peak_log_scaled")
        drop_log = output.get("drop_log")
        if peak_scaled is None or drop_log is None:
            raise ValueError("PointMLP did not return peak-relative heads.")
        peak_log_point = y_scaler.inverse_transform(peak_scaled).reshape(-1)
        sample_index = sample_index.reshape(-1).to(base_scaled.device)
        sample_count = int(sample_index.max().item()) + 1 if sample_index.numel() else 0
        if sample_count > 0:
            counts = torch.bincount(sample_index, minlength=sample_count).to(peak_log_point.dtype).clamp_min(1.0)
            peak_sum = peak_log_point.new_zeros(sample_count).index_add_(0, sample_index, peak_log_point)
            sample_peak_log = peak_sum / counts
            pred_log = sample_peak_log[sample_index].unsqueeze(-1) - F.softplus(drop_log)

    zero_cfg = dict(zero_gate_cfg or {})
    if bool(zero_cfg.get("enabled", False)) and bool(zero_cfg.get("combine_prediction", True)):
        nonzero_logit = output.get("nonzero_logit")
        if nonzero_logit is None:
            raise ValueError("PointMLP did not return zero-gate head.")
        mode = str(zero_cfg.get("combine_mode", "log_gate")).lower()
        p_nonzero = torch.sigmoid(nonzero_logit)
        if mode == "raw_gate":
            pred_raw = p_nonzero * torch.expm1(pred_log).clamp_min(0.0)
            pred_log = torch.log1p(pred_raw)
        else:
            pred_log = p_nonzero * pred_log
    return y_scaler.transform(pred_log)


def _compute_v7_ranking_loss(
    prediction: torch.Tensor,
    target_raw: torch.Tensor,
    sample_index: torch.Tensor,
    loss_cfg: dict[str, Any],
    ...
    weight = float(loss_cfg.get("ranking_loss_weight", 0.0))
    if weight <= 0.0:
        return prediction.new_zeros(())
    pred_log = _scaled_to_log(prediction, y_scaler).reshape(-1)
    target_raw = target_raw.reshape(-1)
    sample_index = sample_index.reshape(-1)
    positive_fraction = _rank_fraction_from_name(str(loss_cfg.get("ranking_positive", "target_top5")), 0.05)
    negative_pred_fraction = float(loss_cfg.get("ranking_negative_pred_fraction", 0.10))
    negative_target_min_rank = float(loss_cfg.get("ranking_negative_target_min_rank_fraction", 0.50))
    low_threshold = float(loss_cfg.get("ranking_negative_low_threshold", 0.0) or 0.0)
    margin = float(loss_cfg.get("ranking_margin_log", math.log(2.0)))
    max_pairs = int(loss_cfg.get("max_pairs_per_sample", 64))
    losses: list[torch.Tensor] = []
    for sample_id in torch.unique(sample_index):
        mask = sample_index == sample_id
        if not bool(mask.any()):
            continue
        local_target = target_raw[mask]
        local_pred_log = pred_log[mask]
        target_rank = _single_sample_rank_fraction(local_target)
        pred_rank = _single_sample_rank_fraction(local_pred_log)
        pos_idx = torch.nonzero(target_rank <= positive_fraction, as_tuple=False).reshape(-1)
        neg_mask = (target_rank >= negative_target_min_rank) & (pred_rank <= negative_pred_fraction)
        if low_threshold > 0.0:
            neg_mask |= (local_target < low_threshold) & (pred_rank <= negative_pred_fraction)
        neg_idx = torch.nonzero(neg_mask, as_tuple=False).reshape(-1)
        if pos_idx.numel() == 0 or neg_idx.numel() == 0:
            continue
        pos_idx = pos_idx[torch.argsort(local_target[pos_idx], descending=True)]
        neg_idx = neg_idx[torch.argsort(local_pred_log[neg_idx], descending=True)]
        if max_pairs > 0:
            pair_side = max(1, int(math.sqrt(max_pairs)))
            pos_idx = pos_idx[:pair_side]
            neg_idx = neg_idx[: max(1, min(int(math.ceil(max_pairs / max(pos_idx.numel(), 1))), neg_idx.numel()))]
        pair_delta = margin + local_pred_log[neg_idx].unsqueeze(0) - local_pred_log[pos_idx].unsqueeze(1)
        pair_loss = F.relu(pair_delta)
        if pair_loss.numel() > max_pairs > 0:
            pair_loss = pair_loss.reshape(-1)[:max_pairs]
...
def _compute_zero_gate_loss(
    output: ModelOutput,
    prediction: torch.Tensor,
    target_raw: torch.Tensor,
    sample_index: torch.Tensor,
    zero_gate_cfg: dict[str, Any],
    y_scaler: StandardScaler,
) -> torch.Tensor:
    cfg = dict(zero_gate_cfg or {})
    if not bool(cfg.get("enabled", False)) or not isinstance(output, dict) or "nonzero_logit" not in output:
        return prediction.new_zeros(())
    threshold = float(cfg.get("zero_threshold", 10.0))
    bce_weight = float(cfg.get("bce_weight", 0.0))
    over_weight = float(cfg.get("overprediction_weight", 0.0))
    zero_cap = float(cfg.get("zero_cap", threshold))
    target_flat = target_raw.reshape(-1)
    logit = output["nonzero_logit"].reshape(-1)
    loss_terms: list[torch.Tensor] = []
    if bce_weight > 0.0:
        label = (target_flat >= threshold).to(logit.dtype)
        loss_terms.append(bce_weight * F.binary_cross_entropy_with_logits(logit, label, reduction="mean"))
    if over_weight > 0.0:
        rank_fraction = _sample_rank_fractions(target_flat, sample_index.reshape(-1).to(target_flat.device))
        zero_mask = (target_flat < threshold) & (rank_fraction >= float(cfg.get("zero_min_rank_fraction", 0.50)))
        if bool(zero_mask.any()):
            pred_log = _scaled_to_log(prediction, y_scaler).reshape(-1)
            cap_log = math.log1p(max(zero_cap, 0.0))
            over_delta = (pred_log[zero_mask] - cap_log).clamp_min(0.0)
            loss_terms.append(over_weight * over_delta.pow(2).mean())
    if not loss_terms:
        return prediction.new_zeros(())
    return torch.stack(loss_terms).sum()


def _compute_peak_relative_loss(
    output: ModelOutput,
    prediction: torch.Tensor,
    target: torch.Tensor,
    target_raw: torch.Tensor,
    sample_index: torch.Tensor,
    peak_relative_cfg: dict[str, Any],
    y_scaler: StandardScaler,
) -> torch.Tensor:
    cfg = dict(peak_relative_cfg or {})
    if not bool(cfg.get("enabled", False)) or not isinstance(output, dict):
        return prediction.new_zeros(())
    peak_scaled = output.get("peak_log_scaled")
    drop_log = output.get("drop_log")
    if peak_scaled is None or drop_log is None:
        return prediction.new_zeros(())
    peak_weight = float(cfg.get("peak_loss_weight", 0.0))
    drop_rank_weight = float(cfg.get("drop_rank_loss_weight", 0.0))
    if peak_weight <= 0.0 and drop_rank_weight <= 0.0:
        return prediction.new_zeros(())
    pred_log = _scaled_to_log(prediction, y_scaler).reshape(-1)
    target_log = _scaled_to_log(target, y_scaler).reshape(-1)
    peak_log_point = _scaled_to_log(peak_scaled, y_scaler).reshape(-1)
    target_raw = target_raw.reshape(-1)
    sample_index = sample_index.reshape(-1)
    losses: list[torch.Tensor] = []
    for sample_id in torch.unique(sample_index):
        ...
        if not bool(mask.any()):
            continue
        sample_peak_pred = peak_log_point[mask].mean().reshape(1)
        sample_peak_target = target_log[mask].max().reshape(1)
        if peak_weight > 0.0:
            losses.append(peak_weight * F.smooth_l1_loss(sample_peak_pred, sample_peak_target, reduction="mean"))
        if drop_rank_weight > 0.0:
            rank = _single_sample_rank_fraction(target_raw[mask])
            top_mask = rank <= _rank_fraction_from_name(str(cfg.get("drop_rank_positive", "target_top5")), 0.05)
            neg_mask = rank >= float(cfg.get("drop_rank_negative_min_rank_fraction", 0.50))
            if bool(top_mask.any()) and bool(neg_mask.any()):
                sample_drop_pred = sample_peak_pred - pred_log[mask]
                target_drop = sample_peak_target - target_log[mask]
                pred_margin = sample_drop_pred[neg_mask].median() - sample_drop_pred[top_mask].median()
                target_margin = target_drop[neg_mask].median() - target_drop[top_mask].median()
                losses.append(drop_rank_weight * F.relu(float(cfg.get("drop_rank_margin_log", 0.0)) + target_margin - pred_margin))
    if not losses:
        return prediction.new_zeros(())
    return torch.stack(losses).mean()
...
def build_model(config: dict[str, Any], input_dim: int) -> PointMLP:
    ...
    zero_gate_cfg = dict(loss_cfg.get("zero_gate", {}))
    peak_relative_cfg = dict(loss_cfg.get("stress_peak_relative", {}))
    output_mode = str(model_cfg.get("output_mode", "auto"))
    model = PointMLP(
        input_dim=input_dim,
        ...
        zero_gate=zero_gate_cfg,
        peak_relative=peak_relative_cfg,
    )
    model.zero_gate_cfg = zero_gate_cfg
    model.peak_relative_cfg = peak_relative_cfg
    model.zero_threshold = float(zero_gate_cfg.get("zero_threshold", 10.0))
    model.zero_cap = float(zero_gate_cfg.get("zero_cap", model.zero_threshold))
    return model
...
def _load_checkpoint_flexible(
    ...
    checkpoint_schema = dict(checkpoint.get("feature_schema", {}))
    schema_matches = checkpoint_schema.get("feature_names") == current_feature_schema.get("feature_names")
    checkpoint_state = checkpoint["model_state"]
    current_state = model.state_dict()
    copied = 0
    skipped = 0
    ...
    for name, tensor in checkpoint_state.items():
        if name not in adapted_state or adapted_state[name].shape != tensor.shape:
            skipped += 1
            continue
        adapted_state[name].copy_(tensor)
        copied += 1
    ...
    if schema_matches:
        if skipped and not allow_partial:
            ...
            return {
                "copied_tensors": len(checkpoint_state) - len(unexpected),
                "skipped_tensors": len(unexpected) + len(missing),
                "partial_first_layer_features": 0,
                "old_input_dim": int(checkpoint_schema.get("input_dim", 0) or 0),
                "current_input_dim": int(current_feature_schema.get("input_dim", 0) or 0),
            }
        model.load_state_dict(adapted_state)
        return {
            "copied_tensors": copied,
            "skipped_tensors": skipped + max(len(current_state) - copied, 0),
            "partial_first_layer_features": 0,
            "old_input_dim": int(checkpoint_schema.get("input_dim", 0) or 0),
            "current_input_dim": int(current_feature_schema.get("input_dim", 0) or 0),
        }
    ...
    if allow_partial:
        return _load_checkpoint_for_feature_schema_change(
            model,
            checkpoint=checkpoint,
            current_feature_schema=current_feature_schema,
        )
    raise ValueError(
    ...
    for loader_step, host_batch in enumerate(loader, start=1):
        ...
        for chunk in host_chunks:
            ...
            prediction = _effective_prediction_scaled(
                ...
                peak_relative_cfg=(loss_cfg or {}).get("stress_peak_relative") if loss_cfg else None,
                zero_gate_cfg=(loss_cfg or {}).get("zero_gate") if loss_cfg else None,
            )
            ...
            if loss_cfg:
                ...
                v7_ranking_loss = _compute_v7_ranking_loss(
                    prediction=prediction,
                    target_raw=target_raw_chunk,
                    sample_index=sample_index_chunk,
                    loss_cfg=loss_cfg,
                ...
                zero_gate_loss = _compute_zero_gate_loss(
                    output=output,
                    prediction=prediction,
                    target_raw=target_raw_chunk,
                    sample_index=sample_index_chunk,
                ...
                peak_relative_loss = _compute_peak_relative_loss(
                    output=output,
                    prediction=prediction,
                    target=target,
                    target_raw=target_raw_chunk,
                ...
                loss = loss + aux_loss + low_target_loss + rank_loss + v7_ranking_loss + zero_gate_loss + peak_relative_loss
            loss.backward()
            if grad_clip > 0.0:
...
def _decode_prediction(prediction_scaled: ModelOutput, y_scaler: StandardScaler) -> tuple[torch.Tensor, torch.Tensor]:
    prediction_scaled = _model_output_tensor(prediction_scaled)
    ...
    with torch.no_grad():
        for loader_step, host_batch in enumerate(loader, start=1):
            ...
            for chunk in _point_chunks(host_batch, point_batch_size=point_batch_size, shuffle=False):
                ...
                predictions_scaled.append(prediction.detach().cpu())
            ...
            for sample_idx in range(len(host_batch.names)):
                ...
                sample_count += 1
                sample_v7_metrics = _false_peak_metrics(
                    sample_pred_log,
                    sample_target,
                    zero_threshold=float(getattr(model, "zero_threshold", 10.0)),
                    zero_cap=float(getattr(model, "zero_cap", 10.0)),
                )
...
class NodeMLPTrainer:
    def __init__(self, config: dict[str, Any], device: torch.device) -> None:
        ...
        self.logger = make_logger(self.save_dir, logger_name="train7")
    ...
    def _prepare(self) -> None:
        ...
        if init_checkpoint:
            ...
            if checkpoint_path.exists():
                ...
                init_summary = _load_checkpoint_flexible(
                    self.model,
                    checkpoint=checkpoint,
                    current_feature_schema=self.feature_schema,
                    allow_partial=bool(self.training_cfg.get("allow_partial_init_checkpoint", False)),
                )
                self.logger.info("Checkpoint init summary: %s", json.dumps(init_summary, ensure_ascii=False))
            else:
