...
from train7.trace_node_frequency_response import (
    _load_model,
    _mode_frequencies,
    _node_label,
    _predict_sample,
    ...
    for sample_path in sample_paths:
        ...
        with torch.no_grad():
            for start in range(0, prepared.num_points, point_batch_size):
                ...
                features = prepared.features[start:stop].to(device)
                output = model(features)
                prediction_scaled = _effective_prediction_scaled(
                    output,
                    y_scaler=y_scaler,
                    sample_index=torch.zeros(features.shape[0], dtype=torch.long, device=device),
...
def run_frequency_curve_diagnostics(
    *,
    ...
    if logger is None:
        logger = make_logger(
            output_dir,
            logger_name="train7.sample_frequency_curves",
